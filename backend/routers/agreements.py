...
class AgreementBase(BaseModel):
    ...
    care_seeker_user_id: Optional[str] = None
...
@router.get("/check-existing", dependencies=[Depends(security)])
def check_existing_agreement(
    care_request_id: Optional[str] = Query(None, description="Care request ID"),
    caregiver_user_id: Optional[str] = Query(None, description="Caregiver user ID"),
    care_seeker_user_id: Optional[str] = Query(None, description="Care seeker user ID"),
    credentials: HTTPAuthorizationCredentials = Depends(security)
):
    """
    Check if an agreement already exists for the given care request, caregiver, and care seeker
    """
    user_id = get_authenticated_user_id(credentials)
    if not user_id:
        raise HTTPException(status_code=401, detail="Unauthorized")
    ...
    try:
        ...
        if care_request_id:
            query = query.eq("care_request_id", care_request_id)
        if caregiver_user_id:
            query = query.eq("caregiver_user_id", caregiver_user_id)
        if care_seeker_user_id:
            query = query.eq("care_seeker_user_id", care_seeker_user_id)
        ...
        if not response.data:
            return {
                ...
                "exists": False,
                "agreement": None,
                "message": "No existing agreement found"
            }

        # Return the most recent agreement
        existing_agreements = response.data
        most_recent = max(existing_agreements, key=lambda x: x.get('created_on', ''))

        return {
            "success": True,
            "exists": True,
            "agreement": most_recent,
            "message": "An agreement already exists for the given parameters"
        }

    except Exception as e:
        print(f"Error checking existing agreement: {str(e)}")
